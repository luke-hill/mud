...
(51..57).each do |id_number|
  ap.seed(
    id: id_number,
    description: 'This secluded area is notorious for being prime looting territory for local '\
                 'Bandits. You notice a lot of large structures to the North, that seems to form '\
                 'a large town or city.',
    advanced_description: 'The path seems to be covered in less Aurel Bark than '\
                          'earlier. Indicating that this path is perhaps used more. You can also hear what sounds like '\
                          'villagers, most likely from the neighbouring Sovereign City.',
    location_id: 'forest_path',
    type: 'Outside'
  )
end
...
ap.seed(
  id: 59,
  description: 'The guards at this gatehouse protect this city well, judging by how pristine '\
...
(61..63).each do |id_number|
  ap.seed(
    ...
    advanced_description: 'A Guard is in the corner, just in case things get out of hand. '\
                          'You see a Portcullis which can release various adversaries one at a time.',
    location_id: 'newtown_training_area',
    type: 'Arena'
  )
end
