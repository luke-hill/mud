...
[21, 22, 24, 25, 27].each do |id_number|
  ap.seed(
    id: id_number,
    description: 'The rocky street here looks unfinished, you notice a cable on the floor '\
'next to the main road.',
    advanced_description: 'Once destined to be the worlds first eco-friendly housing estate, The '\
'Newtown Residential District lingers unfinished, with construction equipment strewn everywhere.',
...
'the main road. There is a well to the east, next to a Small Bank for the Villagers.',
  advanced_description: 'Once destined to be the worlds first eco-friendly housing estate, The '\
'Newtown Residential District lingers unfinished. The sign for the bank looks rusty '\
'and dated as it flaps about in the wind.',
  location_id: 'newtown_residential_district',
  type: 'Town',
  enemy_id: 'no_enemy'
)
...
(29..33).each do |id_number|
  ap.seed(
    id: id_number,
    description: 'One of the few remaining houses in Newtown. The interior looks basic, befitting '\
'a town of little to no wealth.',
    advanced_description: 'Whilst Pristine in condition, this house shows no sign of '\
