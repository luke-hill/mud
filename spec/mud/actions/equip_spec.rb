  ...
  describe '#equip' do
    context 'when the item_id is not in the inventory' do
      ...
      it 'cannot equip a missing item' do
        expect { equip_instance.equip }.to raise_error(ArgumentError).with_message("You do not have a #{item_id} in your inventory.")
      end
    end
    ...
    context 'when the item_id is unknown' do
      ...
      it 'cannot equip an invalid item' do
        expect { equip_instance.equip }.to raise_error(ArgumentError).with_message("Cannot classify #{item_id}.")
      end
    end
  end
